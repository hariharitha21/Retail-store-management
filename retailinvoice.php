...
$count=$id=$quan=0;
	if(isset($_SESSION['count']) && isset($_SESSION['id'])	&& isset($_SESSION['quan']))
while($i<=$_SESSION['count'])
{
	if(!empty($_SESSION['id'][$i]))
	{
					$myid = $_SESSION['id'][$i];
					$myquantity = $_SESSION['quan'][$i];
					$query = "SELECT*
						FROM product
						WHERE p_id='$myid'";
	...
	if(isset($_POST['back']))
{
	...
	header("location: http://localhost/retail/retailshop.php");
	exit();
}
...
while($i<=$_SESSION['count'])
{
				...
				$i++;
					...
					header("location: http://localhost/retail/retailtransaction.php");
					exit();
